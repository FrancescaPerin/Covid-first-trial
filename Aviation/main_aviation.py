...
years = list(np.concatenate([([i]*3) for i in ['2015','2016','2017', '2018', '2019', '2020', '2021']],axis=0))
...
df.replace('Germany (until 1990 former territory of the FRG)', 'Germany', inplace=True) #replacing invalid characters


df.dropna(thresh=df.shape[1]-20, axis=0, inplace=True) #remove empty lines corresponding to same country pairing
...
for geo in countries:
	...
	for partner in countries:
		...
		if geo!=partner:
			...
			avg_on_board = df_paired.iloc[:,2:15:3].mean(axis=1).item() # take data from 2015 to 2018 only
			avg_arrivals = df_paired.iloc[:,3:15:3].mean(axis=1).item()
			avg_departures = df_paired.iloc[:,4:15:3].mean(axis=1).item()

			summary_dict[geo][partner] = {'on_board': avg_on_board, 'arrivals': avg_arrivals, 'departures': avg_departures}
