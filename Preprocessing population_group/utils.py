...
def summary_table(table):
	...
	col=['Reference date (dd/mm/yyyy)','Average household size (number of members)','Under age 15 years among all households','Ages 20-64 years among all households']
	...
	if result.empty :
		...
		one_val=df[['Reference date (dd/mm/yyyy)','Average household size (number of members)']]
		...
		if len(result_2.index)>1:
			...
			result_2=pd.DataFrame({'Reference date (dd/mm/yyyy)':[last_sel],'Average household size (number of members)': [avg]})
